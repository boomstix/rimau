	...
	ScrollScene.prototype.addIndicators = function(options) {
		...
		if (controller) {
			var
				...
				$triggerHook = $("<div>trigger" + suffix + "</div>")
								.css({
									position: "fixed",
									overflow: "visible",
			...
			if ($otherhook.length > 0 && $otherhook.data("parent") == cParams.container) {
				...
				if (cParams.vertical) {
					...
					$triggerHook.css({
						...
						left: (cParams.isDocument ? cParams.container.width() : parentOffset.left + cParams.container.width() - $(document).scrollLeft()) - 110 - options.indent,
						width: 80,
						height: "auto",
					...
					$triggerHook.css({
						top: (cParams.isDocument ? cParams.container.height() : parentOffset.top + cParams.container.height() - $(document).scrollTop()) - 80 - options.indent,
